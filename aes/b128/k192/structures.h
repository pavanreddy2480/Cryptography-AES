 ...
 */
void KeyExpansion(unsigned char inputKey[24], unsigned char expandedKeys[208]) {
	// The first 128 bits are the original key
	for (int i = 0; i < 24; i++) {
		expandedKeys[i] = inputKey[i];
	}

	int bytesGenerated = 24; // Bytes we've generated so far
	...
	while (bytesGenerated < 208) {
		...
		*/
		...
		for (unsigned char a = 0; a < 4; a++) {
			expandedKeys[bytesGenerated] = expandedKeys[bytesGenerated - 24] ^ tmpCore[a];
			bytesGenerated++;
		}
